...
class AffordanceEnv(SingleArmEnv):
    """
    ...
    def reward(self, action=None):
        ...
        # use a shaping reward
            ...
            dist = np.linalg.norm(gripper_site_pos - pot_bottom_height)
            reaching_reward = 1 - np.tanh(10.0 * dist)
            reward += reaching_reward

            # grasping reward
            if self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.pot):
                reward += 0.25
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            @sensor(modality=modality)
            def gripper_to_handle0(obs_cache):
                return (
                    obs_cache["handle0_xpos"] - obs_cache[f"{pf}eef_pos"]
                    if "handle0_xpos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            @sensor(modality=modality)
            def gripper_to_handle1(obs_cache):
                return (
                    obs_cache["handle1_xpos"] - obs_cache[f"{pf}eef_pos"]
                    if "handle1_xpos" in obs_cache and f"{pf}eef_pos" in obs_cache
                    else np.zeros(3)
                )

            sensors = [pot_pos, pot_quat, handle0_xpos, handle1_xpos, gripper_to_handle0, gripper_to_handle1]
    ...
    def visualize(self, vis_settings):
        ...
        if vis_settings["grippers"]:
            self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.pot)
